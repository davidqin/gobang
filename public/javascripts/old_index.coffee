$(document).ready ->
  ...
  socket.on "roomList", (data) ->
    ...
    for room in data.roomList
      html += "<hr><p> Room index :#{room.id}, players :#{room.players}</p>"
  ...
  socket.on "checkIn", (data) ->
    ...
    html += "<button roomId=\"#{data.roomId}\" fun=\"out\" playerId=\"#{data.playerId}\">Exit</button>"
    $('#notice').html(html)
    $('div').hide()
    $('#notice').show()

  $("button[fun='out']").live 'click', ->
    socket.emit 'checkOut', roomId: $(this).attr('roomId'), playerId: $(this).attr('playerId')
